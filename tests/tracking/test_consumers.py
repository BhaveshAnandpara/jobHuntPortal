...
def test_run_sync_enforces_a_hard_timeout(monkeypatch) -> None:
    """`_run_sync` (the one shared sync entry point behind all seven
    `handle_*` functions here) must not let a hanging handler block this
    thread forever — `asyncio.wait_for`'s `DEFAULT_HANDLER_TIMEOUT_SECONDS`
    ceiling (see that constant's docstring) must fire as a plain
    `TimeoutError` so `EventConsumer`'s existing retry/DLQ path can take
    over. Exercised once, through `handle_job_discovered`, since every
    other `handle_*` funnels through the identical `_run_sync` helper.
    Patches the ceiling to a tiny value so this test does not actually
    wait minutes.
    """
    import asyncio
